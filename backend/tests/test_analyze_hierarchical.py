...
async def test_hierarchical_context_integration(mock_db_session):
    ...
    with patch("app.core.analyze_core.rag_service") as mock_rag, \
         ...
         mock_rag.build_hierarchical_context = AsyncMock(return_value=expected_context)
         ...
         await analyzer.analyze_step(note, user, mock_db_session, memory_service=mock_memory)
         ...
         assert "User Identity (Core Traits): IdentityContent" in kwargs['user_context']
...
async def test_build_hierarchical_context(mock_db_session):
    ...
    mock_notes = [Note(title=f"Old {i}", summary="Sum", created_at=datetime.datetime.now()) for i in range(5)]
    mock_res = MagicMock()
    mock_res.scalars = MagicMock()
    mock_res.scalars.return_value.all.return_value = mock_notes
    ...
    with patch.object(rag_service, 'get_medium_term_context', new_callable=AsyncMock) as mock_med, \
         patch.object(rag_service, 'get_long_term_memory', new_callable=AsyncMock) as mock_long:
         
         mock_med.return_value = {"vector": "MediumContent", "graph": "GraphContent"}
         mock_long.return_value = "LongContent"
         
         ctx = await rag_service.build_hierarchical_context(note, mock_db_session, None)
         
         assert "Short-term context (Recent 10 notes):" in ctx
         assert "Sum" in ctx
