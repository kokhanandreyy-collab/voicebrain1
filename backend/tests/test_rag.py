...
async def test_embed_note(db_session, mock_ai_service):
    note = Note(id="note_1", title="Title", summary="Summary", transcription_text="Text", tags=["tag"])
    db_session.add(note)
    ...
    mock_emb = NoteEmbedding(note_id=note.id, embedding=[0.1]*1536)
...
async def test_medium_term_retrieval(db_session, test_user, mock_ai_service):
    ...
    n1 = Note(id="n1", user_id=test_user.id, title="Reference", summary="Reference summary")
    n2 = Note(id="n2", user_id=test_user.id, title="Related", summary="Related summary")
    db_session.add_all([n1, n2])
    await db_session.commit()
    
    # Add embeddings
    emb1 = NoteEmbedding(note_id=n1.id, embedding=[0.1]*1536)
    emb2 = NoteEmbedding(note_id=n2.id, embedding=[0.2]*1536)
    db_session.add_all([emb1, emb2])
    
    # Add relation
    rel = NoteRelation(note_id1=n1.id, note_id2=n2.id, relation_type="related", strength=0.8)
    db_session.add(rel)
    await db_session.commit()
...
async def test_long_term_memory_retrieval(db_session, test_user, mock_ai_service):
    m1 = LongTermMemory(user_id=test_user.id, summary_text="Crucial info", importance_score=10.0, embedding=[0.1]*1536)
    m2 = LongTermMemory(user_id=test_user.id, summary_text="Minor info", importance_score=2.0, embedding=[0.9]*1536)
