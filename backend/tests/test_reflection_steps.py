...
async def test_multi_step_reflection_logic():
    ...
    notes_res = MagicMock()
    notes_res.scalars.return_value.all.return_value = [n1]
    
    # DB calls: count_notes, count_rels, user, notes, commits
    db_mock.execute.side_effect = [
        MagicMock(scalar=lambda: 1), # total notes
        MagicMock(scalar=lambda: 0), # total rels
        user_res,
        notes_res
    ]
    ...
    mock_ai.get_chat_completion.side_effect = [
        '{"facts_summary": "Met John", "importance_score": 8.0}', # Step 1
        '{"stable_identity": "Friendly", "volatile_preferences": {"mood": "positive"}}', # Step 2
        '[{"note1_id": "n1", "note2_id": "n2", "relation_type": "related", "strength": 0.8}]' # Step 3
    ]
